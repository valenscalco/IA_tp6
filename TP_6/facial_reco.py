...
class Perceptron:
    ...
    def suma_oculta(self, z):
        ...
        for g in range(0, pesos_oculto, 7681):
            x += (self.w[g] * self.valor_w0)
            for i in range(7680):
                x += (self.w[g+1] * self.valores_e[i])
            x = x/1000
            valor_y = np.float64(1 / (1 + np.exp(-x)))
            #valor_y = round(valor_y, 10)
            y.append((valor_y)/100)
            # print('suma:', valor_y)
    ...
    def suma_salida(self, y_oculta, pesos_oculto):
        ...
        #valor_suma = round(valor_suma, 10)
        #valor_suma = np.float64(valor_suma)
        #print(valor_suma)
        valor_ys = np.float64(1 / (1 + np.exp(-valor_suma)))
        #valor_ys = round(valor_ys, 10)
        #print('suma salida:', valor_ys)
        y_oculta.append((valor_ys)/10)
        return (y_oculta)
    ...
    def resolucion(self):
        ...
        for i in range(self.resp+1):
            if i == self.resp:
                ...
                for j in range(7681):
                    self.w.append(round(random.uniform(-1, 1), 2))
        pesosexport = open('pesosiniciales.txt', 'w')
        ...
        for t in range(0, self.it):
            for i in range(0, 10):
                self.valores_e = leerimage.leer_images(i)
